...
type QuestionDetail = {
  IDCauHoi: string;
  IDBaiHoc: string;
  NoiDungCauHoi: any; // json string tiptap
...
/**
 * ✅ PATCH lựa chọn theo đúng API của sếp:
 * PATCH /api/lessons/lua-chon/{luachonId}/
 */
async function patchChoice(luachonId: string, payload: ChoiceApi) {
  return http.patch(`/api/lessons/lua-chon/${luachonId}/`, payload);
}
...
export default function EditQuestionPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const finalWords = normalizedLoai === "sapxeptu" ? ensureAtLeast2Words(finalId, mappedWords) : [];

        const finalPairs = normalizedLoai === "khoptu" ? ensureAtLeast2Pairs(finalId, mappedPairs) : [];
  ...
  const onSubmit = forms.handleSubmit(async (values) => {
    ...
    try {
      ...
      if (type === "nghe") {
        payloadQuestion.FileNghe_url = values.FileNghe_url || null;
      }

      await updateQuestion(id, payloadQuestion);

      // 2) ✅ PATCH từng lựa chọn theo API riêng
      if (type === "tracnghiem" || type === "nghe") {
        const list = ensureAtLeast2Choices(id, values.lua_chon_data || []).map((c, i) => ({
          ...c,
          LuaChonID: c.LuaChonID || buildChoiceId(id, i + 1),
        }));

        // ✅ đảm bảo chỉ 1 đáp án đúng (phòng khi UI bug)
        const idxTrue = list.findIndex((x) => x.DapAnDung);
        const trueIndex = idxTrue === -1 ? 0 : idxTrue;
        list.forEach((x, idx) => (x.DapAnDung = idx === trueIndex));

        // PATCH tuần tự để thấy rõ trong Network
        for (const c of list) {
          await patchChoice(c.LuaChonID, {
            LuaChonID: c.LuaChonID,
            CauHoiID: id,
            NoiDungLuaChon: c.NoiDungLuaChon,
            DapAnDung: c.DapAnDung,
          });
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_4px_0_0_rgba(143,156,173,0.2)] space-y-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <button
                  ...
                  onClick={() => {
                    const qid = (questionIdFromUrl || idCauHoiInForm || "").trim();
                    choiceFA.append({
                      LuaChonID: buildChoiceId(qid, choiceFA.fields.length + 1),
                      NoiDungLuaChon: "",
                      DapAnDung: false,
                    });
                  }}
              ...
              <div className="space-y-3">
                {choiceFA.fields.map((f, idx) => {
                  const qid = (questionIdFromUrl || idCauHoiInForm || "").trim();
                  const autoId = (f as any).LuaChonID || buildChoiceId(qid, idx + 1);
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            current.forEach((c, i) => {
                              choiceFA.update(i, {
                                ...c,
                                LuaChonID: c.LuaChonID || buildChoiceId(qid, i + 1),
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_4px_0_0_rgba(143,156,173,0.2)] space-y-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    wordFA.append({ IDTu: buildWordId(qid, n), NoiDungTu: "", ThuTuDung: n });
                  }}
                >
                  + Thêm từ
                </button>
              ...
              <div className="space-y-3">
                {wordFA.fields.map((f, idx) => {
                  const qid = (questionIdFromUrl || idCauHoiInForm || "").trim();
                  const wordId = (f as any).IDTu || buildWordId(qid, idx + 1);
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_4px_0_0_rgba(143,156,173,0.2)] space-y-4">
              ...
              <div className="space-y-3">
                {pairFA.fields.map((f, idx) => {
                  const qid = (questionIdFromUrl || idCauHoiInForm || "").trim();
                  const pairId = (f as any).IDCapTu || buildPairId(qid, idx + 1);
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            pairFA.update(idx, {
                              ...
                              IDCapTu: (forms.getValues(`cap_tu_khop_data.${idx}.IDCapTu`) as any) || pairId,
...
// // PATCH câu hỏi (không update choices ở đây)
// async function updateQuestion(id: string, payload: any) {
//   return http.patch(`/api/lessons/cau-hoi/${id}/`, payload);
// }

// // ✅ PATCH lựa chọn theo API mới
// async function patchChoice(luachonId: string, payload: ChoiceApi) {
//   return http.patch(`/api/lessons/lua-chon/${luachonId}/`, payload);
// }
...
//   /** ✅ set đúng 1 đáp án đúng (ổn định, không loop update nhiều lần) */
//   const setSingleCorrectChoice = (idxTrue: number) => {
//     const list = forms.getValues("lua_chon_data") || [];
//     const next = list.map((c, i) => ({ ...c, DapAnDung: i === idxTrue }));
//     forms.setValue("lua_chon_data", next, { shouldDirty: true, shouldValidate: true });
//     choiceFA.replace(next);
//   };
...
//       // 1) PATCH question (KHÔNG gửi lua_chon_data nữa)
//       const payloadQuestion: any = {
//         IDCauHoi: id,
//         IDBaiHoc: values.IDBaiHoc,
//         NoiDungCauHoi: contentStr,
...
//         GiaiThich: values.GiaiThich?.trim() ? values.GiaiThich.trim() : null,
//       };
//       if (type === "nghe") payloadQuestion.FileNghe_url = values.FileNghe_url || null;

//       await updateQuestion(id, payloadQuestion);

//       // 2) PATCH từng lựa chọn theo API mới
//       if (type === "tracnghiem" || type === "nghe") {
//         const list: ChoiceApi[] = (values.lua_chon_data || []).map((c, i) => ({
//           LuaChonID: c.LuaChonID || buildChoiceId(id, i + 1),
//           CauHoiID: id,
//           NoiDungLuaChon: c.NoiDungLuaChon,
//           DapAnDung: !!c.DapAnDung,
...
//         await Promise.all(
//           list.map((c) => {
//             if (!c.LuaChonID) throw new Error("Thiếu LuaChonID để cập nhật lựa chọn.");
//             return patchChoice(c.LuaChonID, c);
//           })
//         );
//       }
...
//                     Số lựa chọn: <b>{choiceFA.fields.length}</b> • (API update:
//                     <code className="ml-1">/api/lessons/lua-chon/:LuaChonID/</code>)
//                   </div>
//                 </div>
...
//                   onClick={() => {
//                     const qid = (questionIdFromUrl || idCauHoiInForm || "").trim();
//                     // ⚠️ chỉ PATCH được nếu backend đã có LuaChonID tương ứng.
//                     // Nếu backend có POST tạo lựa chọn, mình nên gọi POST rồi append bằng id backend trả về.
//                     choiceFA.append({
//                       LuaChonID: buildChoiceId(qid, choiceFA.fields.length + 1),
//                       NoiDungLuaChon: "",
//                       DapAnDung: false,
//                     });
//                     // đảm bảo vẫn có ít nhất 1 đáp án đúng
//                     if (!(forms.getValues("lua_chon_data") || []).some((x) => x.DapAnDung)) {
//                       setSingleCorrectChoice(0);
//                     }
//                   }}
//                 >
//                   + Thêm lựa chọn
...
//                 {choiceFA.fields.map((f, idx) => {
//                   const qid = (questionIdFromUrl || idCauHoiInForm || "").trim();
//                   const choiceId = (f as any).LuaChonID || buildChoiceId(qid, idx + 1);
...
//                           className="size-5 cursor-pointer"
//                           checked={!!forms.getValues(`lua_chon_data.${idx}.DapAnDung`)}
//                           onChange={() => setSingleCorrectChoice(idx)}
//                         />
//                         <span className="font-semibold text-slate-700">Đúng</span>
//                       </div>
...
//                         className="ml-auto mt-auto h-10 rounded-xl border border-red-200 bg-red-50 px-4 font-semibold text-red-700 hover:bg-red-100 disabled:opacity-60"
//                         onClick={() => {
//                           // không cho xóa nếu còn 1 lựa chọn
//                           if (choiceFA.fields.length <= 1) return;

//                           const list = forms.getValues("lua_chon_data") || [];
//                           const removingIsTrue = !!list[idx]?.DapAnDung;

//                           choiceFA.remove(idx);

//                           // nếu xóa mất đáp án đúng -> set cái đầu đúng
//                           setTimeout(() => {
//                             const next = forms.getValues("lua_chon_data") || [];
//                             if (removingIsTrue && next.length) {
//                               const normalized = next.map((c, i) => ({
//                                 ...c,
//                                 DapAnDung: i === 0,
//                               }));
//                               forms.setValue("lua_chon_data", normalized, {
//                                 shouldDirty: true,
//                                 shouldValidate: true,
//                               });
//                               choiceFA.replace(normalized);
//                             }
//                           }, 0);
//                         }}
//                         disabled={choiceFA.fields.length <= 1}
