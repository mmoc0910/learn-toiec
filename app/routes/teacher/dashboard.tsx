...
type Row = {
  ...
  // số câu đúng theo phần (phục vụ chart 3 phần)
  ghepTu: number;
  doc: number;
  nghe: number;
...
// Mapping loại câu hỏi -> bucket (3 phần cho chart)
...
export default function Dashboard() {
  ...
  const myResults = useMemo(() => {
    ...
    return (results || []).filter(
      (r) => r?.DeThiID_detail?.IDGiaoVien === myId
    );
  }, [results, user?.IDTaiKhoan]);

  // ✅ build rows table (mỗi lượt thi = 1 dòng) -> dùng cho chart
  ...
  const examTableRows: ExamTableRow[] = useMemo(() => {
    ...
    for (const r of myResults) {
      ...
      // ✅ Lớp (fallback —). Nếu backend khác key, sếp sửa dòng này.
      const tenLop =
  ...
  const exportExamPdf = async (exam: ExamTableRow) => {
    try {
      const byExam = myResults.filter(
        (r) => (r?.DeThiID || "—") === exam.deThiId
      );
      ...
      // ✅ FIX FONT: load Roboto Condensed into VFS then setFont
      await ensureVietnameseFont(docPdf);
      docPdf.setFont("RobotoCondensed", "normal");

      // ===== Header giống mẫu =====
      docPdf.setFont("RobotoCondensed", "bold");
    ...
    <ContentLayoutWrapper heading="Tổng quan">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="col-span-2 rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_4px_0_0_rgba(143,156,173,0.2)] mb-6">
          ...
          <div className="mt-3 text-xs text-slate-500">
            *Các thống kê nâng cao cần API khác, hiện đang lấy từ /ket-qua/ để
            vẽ biểu đồ.
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="lg:col-span-3 rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_4px_0_0_rgba(143,156,173,0.2)]">
          <div className="mb-2">
            <div className="text-lg font-extrabold text-slate-900">
              Xu hướng theo đề (Line)
            </div>
            <div className="text-sm text-slate-500">
              % đúng TB và số câu đúng theo phần (Ghép từ / Đọc / Nghe)
            </div>
          </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_4px_0_0_rgba(143,156,173,0.2)]">
        <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
          <div>
            ...
            <div className="text-sm text-slate-500">
              Mỗi dòng = 1 đề · Hiển thị lớp, giáo viên, 5 học viên · Xuất PDF
              theo mẫu
            </div>
          </div>
        </div>
