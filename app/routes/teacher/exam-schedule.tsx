...
type ExamQuestionDetail = {
  ...
  // ✅ 2 loại mới (backend có thể trả về)
  tu_sap_xep?: { IDTu: string; CauHoiID?: string; NoiDungTu: string; ThuTuDung: number }[];
  cap_tu_khop?: { IDCapTu: string; CauHoiID?: string; TuBenTrai: string; TuBenPhai: string; LaDung: boolean }[];
};
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="mt-2 flex flex-wrap gap-2">
                              ...
                              <span className="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">
                                🕒 {dayjs(it.ThoiGianBatDau).format("DD/MM/YYYY HH:mm")} →{" "}
                                {dayjs(it.ThoiGianKetThuc).format("DD/MM/YYYY HH:mm")}
                              </span>
                            </div>
                          </div>
                    ...
                    <div className="mt-4 space-y-3">
                      {(selectedSchedule.IDDeThi_detail?.chi_tiet || [])
                        ...
                        .map((ct) => {
                          ...
                          // ✅ data 2 loại mới (fallback any để tránh crash TS nếu backend chưa có type)
                          const tuSapXep: any[] = (q as any)?.tu_sap_xep || [];
                          const capTuKhop: any[] = (q as any)?.cap_tu_khop || [];

                          const sortedWords = tuSapXep
                            .slice()
                            .sort((a, b) => (a.ThuTuDung ?? 0) - (b.ThuTuDung ?? 0));

                          const correctSentence =
                            sortedWords.length > 0
                              ? sortedWords.map((w) => w.NoiDungTu).join(" ")
                              : "";
                          ...
                          return (
                            ...
                            >
                              ...
                              {/* ✅ hiển thị lựa chọn chỉ cho nghe + trắc nghiệm */}
