...
// ✅ stringify để fallback hiển thị nội dung trong modal xem
function safeStringifyPretty(v: any) {
  if (v == null) return "";
...
export default function Lessons() {
  ...
  const [viewLesson, setViewLesson] = useState<LessonUI | null>(null);
  ...
  /** =======================
   *  Lesson: OPEN VIEW MODAL
   *  ======================= */
  const openViewLesson = (lesson: LessonUI) => {
    setErrorMsg(null);
  ...
  const viewContent = useMemo(() => {
    if (!viewLesson) return null;
    return safeParseTiptapContent(viewLesson.raw.NoiDungBaiHoc);
  }, [viewLesson]);
        ...
        <Modal
          ...
          footer={
            <div className="flex items-center justify-end">
              <button
                type="button"
                onClick={closeViewLesson}
                className="h-11 rounded-xl border border-slate-200 bg-white px-5 font-semibold text-slate-700 hover:bg-slate-50"
              >
                Đóng
              </button>
            </div>
          ...
          {!viewLesson ? (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4">
                  ...
                  <div className="mt-1 text-sm font-semibold text-slate-800">
                    {viewLesson.raw.IDChuDe_detail || viewLesson.raw.IDChuDe}
                  </div>
                </div>
                ...
                {/* <div className="rounded-2xl border border-slate-200 bg-white p-4">
                  ...
                  <div className="mt-1 text-sm font-semibold text-slate-800">
                    {Array.isArray(viewLesson.raw.cau_hoi)
                      ? viewLesson.raw.cau_hoi.length
                      : 0}
                  </div>
                </div> */}
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                {/* ✅ Nếu SimpleEditor của sếp hỗ trợ readOnly/disabled thì dùng luôn.
                    Nếu không support, nó vẫn render nhưng có thể cho edit -> fallback bên dưới để an toàn.
                */}

                {/* ✅ Fallback hiển thị text nếu editor không đọc được */}
                <div className="mt-3">
                  {/* <div className="text-xs font-semibold text-slate-500 mb-1">
                    (Fallback hiển thị nội dung)
                  </div> */}
                  <pre className="max-h-[340px] overflow-auto rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-800">
                    {/* {safeStringifyPretty(viewContent)} */}
                    <TiptapViewer content={viewContent}></TiptapViewer>
                  </pre>
                </div>
              </div>
            </div>
