...
export default function Dashboard() {
  ...
  const examSummaryRows: ExamSummaryRow[] = useMemo(() => {
    ...
    for (const { examId, attempts } of mp.values()) {
      ...
      const tenDe =
        ex?.TenDeThi ||
        (resultsRes?.results || []).find((x) => x.DeThiID === examId)?.DeThiID_detail
          ?.TenDeThi ||
        examId;
          ...
          <div className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-3">
              <div className="text-lg font-bold text-slate-800">
                Xu hướng lượt làm theo đề (Top 10)
              </div>
              <div className="text-sm text-slate-500">LineChart theo số lượt làm (count)</div>
            </div>

            <div className="h-64">
              <ResponsiveContainer>
                <LineChart data={examAttemptsBarData} margin={{ left: 10, right: 10 }}>
